...
#         return res.json({"craftflips": craftflips, "refresh_session": not session.active})

def setup(app: flask.Flask):
    @app.route('/craftflips')
    def craftflip_endpoint():
        return res.json({"craftflips": cogs.core.craftflip_engine.craftflips})