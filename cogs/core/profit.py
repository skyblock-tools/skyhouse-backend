...
def find_flips():
    ...
    for item in items:
        l_bins = runtimeConfig.redis.zrevrangebyscore(item, 0, math.inf, withscores=True)
        flip = False
        if len(l_bins) >= 5:

            auctions = runtimeConfig.redis.zrevrangebyscore(f"auctions:{item[5:]}", 0, math.inf, start=0, num=5, withscores=True)
